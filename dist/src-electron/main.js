...
const LOCAL_MANIFEST = path.join(app.getPath('userData'), 'assets-manifest.json');
const MANIFEST_URL = 'https://github.com/boltnoak/boltnotes-assets/releases/latest/download/manifest.json';
...
const DOCUMENTS = path.join(app.getPath('documents'), 'BoltNotes');
const APPDATA = path.join(app.getPath('userData'));
const COVERS = path.join(APPDATA, 'game-covers');
const MEDIA_DIR = path.join(DOCUMENTS, 'Notes', 'Media');
const GAMELOGOS = path.join(APPDATA, 'game-logos');
const HEROS = path.join(APPDATA, 'game-heros');
const USER_COVERS = path.join(DOCUMENTS, 'Games', 'Custom Covers');
const BUNDLE = path.join(__dirname, '..', 'src');
const NOTES_LIST = path.join(DOCUMENTS, 'Notes', '.NotesList');
...
ipcMain.handle('fortnite:fetch-trailers', async () => {
    return await fetchWithCache('https://gist.githubusercontent.com/boltnoak/a836e64254fca6d8263c6d66347e021d/raw/fn-trailers.json', 'fn-trailers.json');
});
ipcMain.handle('fortnite:fetch-seasons', async () => {
    return await fetchWithCache('https://gist.githubusercontent.com/boltnoak/a836e64254fca6d8263c6d66347e021d/raw/fn-seasons.json', 'fn-seasons.json');
});
ipcMain.handle('fortnite:list-trailers', () => {
    return fs.readdirSync(path.join(ASSETS_DIR, 'fortnite/trailers'));
});
ipcMain.on('devTools', () => {
...
ipcMain.handle('games:add', async (_, newGameData) => {
    ...
    try {
        ...
        if (fs.existsSync(achievementsPath)) {
            ...
            if (content.trim())
                achievementsList = JSON.parse(content);
        }
...
ipcMain.handle('games:ensure-cover', async (_, { appid, name, cover }) => {
    ...
    if (!result.hero && fs.existsSync(placeholderPath))
        result.hero = placeholderPath;
    return result;
});
// Temas
